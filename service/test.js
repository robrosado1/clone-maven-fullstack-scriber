...
describe('suggest', () => {
  it("madonna returns ray of light", async () => {
    const response = await chakram.get('http://localhost:80/service/suggest/artists?prefix=madon');
  ...
  it("world tracks", async () => {
    const response = await chakram.get('http://localhost:80/service/suggest/tracks?prefix=world')
    const worldTracks = {
      "suggestions": [
        {
  ...
  it("dream releases", async () => {
    ...
    const response = await chakram.get('http://localhost:80/service/suggest/releases?prefix=dream');
    expect(response).to.have.status(200);
  ...
  it("all zz should give us zzonked and zzino", async () => {
    ...
    const response = await chakram.get('http://localhost:80/service/suggest/all?prefix=zz');
    expect(response).to.have.status(200);
